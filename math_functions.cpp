                               ...
                               size_t input_size, size_t output_size, float* output)
{
    ...
    for (size_t i = 0; i < output_size; ++i) {
        float out_row = 0.0f;
        const float* row_scale = scale_y + i * (input_size / 32);
        const uint32_t* row_add = mask_add + i * (input_size / 32);
        const uint32_t* row_sub = mask_sub + i * (input_size / 32);

        for (size_t j = 0; j < input_size; j += 32) {
            int block_index = j / 32;
            const uint32_t ma = row_add[block_index];
            const uint32_t ms = row_sub[block_index];
            ...
            sum = _mm256_mask_add_epi16(sum, (__mmask16)ma, sum, x_lo);
            sum = _mm256_mask_add_epi16(sum, (__mmask16)(ma >> 16), sum, x_hi);
            sum = _mm256_mask_sub_epi16(sum, (__mmask16)ms, sum, x_lo);
            sum = _mm256_mask_sub_epi16(sum, (__mmask16)(ms >> 16), sum, x_hi);
    ...
    for (size_t i = 0; i < output_size; ++i) {
        ...
        for (size_t j = 0; j < input_size; j += 32) {
            ...
            for (size_t k = 0; k < 32; ++k) {
                ...
                int y = static_cast<int16_t>(x_val & -((ma >> k) & 1)) - 
                        static_cast<int16_t>(x_val & -((ms >> k) & 1));
